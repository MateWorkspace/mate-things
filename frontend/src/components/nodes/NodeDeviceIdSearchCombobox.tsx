...
import { searchNodeDeviceIdsAction } from "@/lib/actions/entity-search-actions";
import type { SearchOption } from "@/lib/actions/search-options";

const RESULTS_PER_PAGE = 6;
const DEBOUNCE_MS = 300;

interface NodeDeviceIdSearchComboboxProps {
  name: string;
...
export default function NodeDeviceIdSearchCombobox({
  name,
  defaultDeviceId,
  defaultNodeName,
  label = "Node",
  placeholder = "Any node",
}: NodeDeviceIdSearchComboboxProps) {
  const id = useId();
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      let cancelled = false;
      setLoading(true);
      searchNodeDeviceIdsAction({ query, page, limit: RESULTS_PER_PAGE })
        .then((result) => {
          if (cancelled) return;
  ...
  function select(node: SearchOption | null) {
    setSelectedDeviceId(node?.value ?? "");
