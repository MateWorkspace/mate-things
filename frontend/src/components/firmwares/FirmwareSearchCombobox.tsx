...
interface FirmwareSearchComboboxProps {
  name: string;
  defaultFirmwareId?: string;
  defaultFirmwareName?: string;
  label?: string;
  placeholder?: string;
}

export default function FirmwareSearchCombobox({
  name,
  defaultFirmwareId,
  defaultFirmwareName,
  label = "Firmware",
  placeholder = "Any firmware",
}: FirmwareSearchComboboxProps) {
  const id = useId();
  const containerRef = useRef<HTMLDivElement>(null);
  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState("");
  const [page, setPage] = useState(1);
  const [firmwares, setFirmwares] = useState<SearchOption[]>([]);
  const [totalPages, setTotalPages] = useState(1);
  const [loading, setLoading] = useState(false);
  const [selectedId, setSelectedId] = useState(defaultFirmwareId ?? "");
  const [selectedName, setSelectedName] = useState(defaultFirmwareName ?? "");
  ...
  function select(firmware: SearchOption | null) {
    setSelectedId(firmware?.value ?? "");
    setSelectedName(firmware?.label ?? "");
    setOpen(false);
  }
          ...
          >
            ...
            {loading ? (
              ...
              firmwares.map((firmware) => (
                <li key={firmware.value}>
                  <button
                    type="button"
                    role="option"
                    aria-selected={firmware.value === selectedId}
                    onClick={() => select(firmware)}
                    className={`hover:bg-highlight/40 w-full rounded-lg px-2.5 py-2 text-left text-sm transition-colors ${
                      firmware.value === selectedId
