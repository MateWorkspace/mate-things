...
interface NodeClassSearchComboboxProps {
  name: string;
  defaultNodeClassId?: string;
  defaultNodeClassName?: string;
  label?: string;
  placeholder?: string;
}

export default function NodeClassSearchCombobox({
  name,
  defaultNodeClassId,
  defaultNodeClassName,
  label = "Node class",
  placeholder = "Any node class",
}: NodeClassSearchComboboxProps) {
  const id = useId();
        ...
        <div className="border-border bg-surface absolute z-20 mt-1.5 w-full min-w-64 rounded-xl border p-2 shadow-lg">
          <div className="relative">
            ...
            <Input
              autoFocus
              className="pl-9"
              placeholder="Search node classes"
              value={query}
              onChange={(event) => {
          ...
          <div className="border-border mt-2 flex items-center justify-between border-t pt-2">
            ...
            <div className="flex gap-1">
              <button
                type="button"
                aria-label="Previous node classes"
