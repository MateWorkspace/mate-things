...
func (p *postgresImpl) queryCreate(
	...
	return p.SqrD.Insert("permissions").
		Columns(columns...).
		Values(values...).
		Suffix("RETURNING id").
		ToSql()
...
func (p *postgresImpl) queryReadById(id uuid.UUID) (query string, args []any, err error) {
	return p.SqrD.Select(permissionColumns...).
		From("permissions").
...
func (p *postgresImpl) queryReadByPagination(
	...
	baseQ := p.SqrD.Select(permissionColumns...).
		From("permissions").
		Where("deleted_at IS NULL")
...
func (p *postgresImpl) queryDeleteById(
	...
	return p.SqrD.Update("permissions").
		Where(squirrel.Eq{"id": id}).
		Where("deleted_at IS NULL").
