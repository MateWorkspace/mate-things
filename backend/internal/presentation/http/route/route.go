...
type AdminHandler interface {
	...
	LlmConfigTestConnectionWithConfigPost(c *echo.Context) error
...
type InfraredHandler interface {
	...
	RecordSessionCoderGetById(c *echo.Context) error
...
func routeAdmin(v1 *echo.Group, handler AdminHandler, permission PermissionMiddleware) {
	...
	v1.POST("/admin/llm-config/test-connection", handler.LlmConfigTestConnectionWithConfigPost, permission("llm_config:set"))
