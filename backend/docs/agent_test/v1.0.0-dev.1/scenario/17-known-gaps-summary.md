...
| 12 | Node | `device_id` on `NodePatchRequest` had zero validation | **Fixed** — device identity is immutable after MQTT registration. The field was removed from the presentation and application update DTOs; unknown `device_id` PATCH input is ignored and cannot reach the repository update. |
...
| 15 | OTA dispatch | The caller supplied `firmware_url`, and firmware compatibility was not authoritative | **Fixed** — the public request now accepts only `firmware_id`; the OTA usecase reads the node and firmware, rejects a node-class mismatch, presigns the authoritative stored binary, and publishes its authoritative URL, size, and checksum. |
